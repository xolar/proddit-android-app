...
public class LoginTask extends AsyncTask<Void, Void, Boolean> {
	...
	private ProdditSettings mSettings;
	private HttpClient mClient;
	private Context mContext;
	
	protected LoginTask(String username, String password, ProdditSettings settings, HttpClient client, Context context) {
		mUsername = username;
		mPassword = password;
		mSettings = settings;
		mClient = client;
     ...
     */
    private Boolean doLogin(String username, String password, ProdditSettings settings, HttpClient client, Context context) {
		String status = "";
    	String userError = "Eroare la logare, va rugam incercati din nou.";
    	...
    	try {
        	...
        	if (jp.nextToken() != JsonToken.END_ARRAY) {
	        	if (line.contains("WRONG_PASSWORD")) {
	        		userError = "Parola si/sau nume utilizator invalide.";
    		...
    		mUserError = userError+" "+e.getMessage();
    		if (entity != null) {
    			try {
    				entity.consumeContent();
    			} catch (Exception e2) {
